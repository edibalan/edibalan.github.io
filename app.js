...
export default class App {
  constructor() {
    ...
    this.renderAudioElements = () => {
      ...
      for (let i = 0; i < this.audioElements.src.length; i++) {
        container += `
          <audio id="${this.audioElements.id[i]}" src="./media/${this.audioElements.src[i]}"
  ...
  async render() {
    ...
    this.loadAnimation.innerHTML = `<img alt="animation" class="animation" src="./media/load-animation.gif" />`;
    ...
    setTimeout(() => {
      ...
      new ProjectsAnimation().initiate();
